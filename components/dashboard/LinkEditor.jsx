    ...
    >
      ...
      <div className="px-3 sm:px-5 py-2 bg-base-200/30 flex flex-col sm:flex-row md:items-center justify-between border-t border-base-200 gap-y-2">
        <div className="flex items-center gap-3 sm:gap-6">
          ...
          >
            <RiUserStarLine className="text-info text-sm" />
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          {!link.is_active && (
            <span className="badge badge-ghost badge-xs font-bold opacity-40">
              Inactive
            </span>
          )}
        </div>
...
export default function LinkEditor({
  ...
  const handleAiTitle = async () => {
    ...
    setIsAiLoading(true);
    try {
      const { data } = await axios.post(ENDPOINTS.AI.GENERATE_LINK_TITLE, {
        url: formData.url,
        plan: plan,
      });
      ...
      if (
        data.success &&
        (data.data?.suggestions?.length > 0 || data.data?.title)
      ) {
        setAiSuggestion(data.data);

        // Optional: Auto-fill if empty
        if (!formData.title) {
          const firstOption = data.data.suggestions
              ...
              <form onSubmit={handleSubmit} className="space-y-2">
                <div className="form-control">
                  <label className="label">
                    <span className="label-text font-bold text-xs uppercase tracking-widest">
                      Link Title
                    </span>
                  </label>
                  <input
                      ...
                      <div className="overflow-hidden">
                        <div className="p-4 border-t border-base-300">
                          ...
                          <div className="grid grid-cols-6 gap-2">
                            {CONFIG.COMMON_EMOJIS.find(
                              (c) => c.name === activeEmojiTab
                            )?.emojis.map((emoji) => (
                              <button
                                key={emoji}
                                type="button"
