    ...
    const plans = [
        {
            ...
            currency: CONFIG.PRICING.PRO.currency,
            icon: RiVipCrownLine,
            color: "primary",
            features: [
                "Unlimited Links",
        ...
        },
        {
            ...
            currency: CONFIG.PRICING.AGENCY.currency,
                ...
                <div className="p-4 md:p-6 grid grid-cols-1 md:grid-cols-2 gap-4 bg-base-50/50 overflow-y-auto flex-1">
                    {plans.map((plan) => {
                        ...
                        return (
                            <div
                                key={plan.id}
                                className={`relative flex flex-col border rounded-md p-4 md:p-5 transition-all duration-200 hover:shadow-md ${isPro
                                    ? 'border-primary/40 bg-primary/[0.02]'
                                    : 'border-base-300 bg-base-100'
                                    }`}
                            >
                                {isPro && (
                                    <div className="absolute -top-2.5 left-1/2 -translate-x-1/2 bg-primary text-primary-content text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-sm shadow-sm">
                                        Most Popular
                                    </div>
                                )}

                                <div className="mb-4 text-center">
                                    <div className={`w-10 h-10 mx-auto rounded-md flex items-center justify-center mb-2 ${isPro ? 'bg-primary/10 text-primary' : 'bg-secondary/10 text-secondary'
                                        }`}>
                                ...
                                <div className="space-y-2 mb-6 flex-1">
                                    {plan.features.map((feature, idx) => (
                                        <div key={idx} className="flex items-start gap-2 text-xs font-medium opacity-80">
                                            <div className={`mt-0.5 p-0.5 rounded-sm ${isPro ? 'bg-primary/20 text-primary' : 'bg-secondary/20 text-secondary'
                                                }`}>
                                ...
                                <button
                                    ...
                                    disabled={loading}
                                    className={`btn w-full btn-md h-10 min-h-0 font-bold border-0 text-white shadow-sm rounded-md hover:opacity-90 ${isPro
