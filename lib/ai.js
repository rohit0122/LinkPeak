    ...
    async generateContent(prompt, options = {}) {
        ...
        if (process.env.GEMINI_API_KEY) {
            try {
                ...
                console.warn("Gemini check failed, moving to next provider...");
            } catch (err) {
        ...
        if (process.env.MISTRAL_API_KEY) {
            try {
                const response = await fetch(CONFIG.AI_ENDPOINTS.MISTRAL, {
                    method: "POST",
                    headers: {
                    ...
                    body: JSON.stringify({
                        ...
                        messages: [{ role: "currentUser", content: prompt }],
                ...
                console.warn("Mistral check failed, falling back to mock...");
            } catch (err) {
