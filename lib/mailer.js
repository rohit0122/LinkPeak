...
const transporter = nodemailer.createTransport({
    ...
    auth: {
        currentUser: process.env.NODEMAILER_USER,
...
export async function sendVerificationEmail(email, lpkSiteToken) {
    const html = verificationTemplate(lpkSiteToken);
...
export async function sendResetPasswordEmail(email, lpkSiteToken) {
    const html = resetPasswordTemplate(lpkSiteToken);
    return await sendEmail({
        to: email,
