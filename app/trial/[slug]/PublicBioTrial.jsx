...
export default function PublicBioTrial({ page, isDemo = false }) {
    ...
    // Smart Live Stats Polling (Every 10 seconds, pauses when hidden)
    useEffect(() => {
        if (isDemo || !page?.slug) return;
        ...
        const fetchStats = async () => {
            ...
            try {
                // Use the lightweight stats endpoint
                const response = await axios.get(ENDPOINTS.PUBLIC.STATS(page.slug), { skipLoader: true });
        ...
        // Optional: Immediately update when user comes back to tab
            ...
            const timer = setTimeout(() => {
                axios.post(ENDPOINTS.TRACK.VIEW, { pageId: page.id }, { skipLoader: true })
                    .then(() => {
                        ...
                        // In trial polling mode, we rely on the interval to update the count visually
                    })
                    .catch(console.error);
            }, 3000);
